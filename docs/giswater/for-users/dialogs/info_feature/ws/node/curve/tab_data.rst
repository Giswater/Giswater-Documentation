...
.. raw:: html
	...
	<details>
		...
		<ul>
			...
			<li><strong>Dvquerytext:</strong> Los valores de este desplegable estan determinados por la consulta:
				<code>
					SELECT function_type as id, function_type as idval FROM man_type_function WHERE ((featurecat_id is null AND 'NODE'=ANY(feature_type)) ) AND active IS TRUE  OR 'CURVE' = ANY(featurecat_id::text[])
				</code>
			</li>
			<li>
				<details class='no-square'>
...
.. raw:: html
	...
	<details>
		...
		<ul>
			...
			<li><strong>Dvquerytext:</strong> Los valores de este desplegable estan determinados por la consulta:
				<code>
					SELECT category_type as id, category_type as idval FROM man_type_category WHERE ((featurecat_id is null AND 'NODE'=ANY(feature_type))) AND active IS TRUE  OR 'CURVE' = ANY(featurecat_id::text[])
				</code>
			</li>
			<li>
				<details class='no-square'>
...
.. raw:: html
	...
	<details>
		...
		<ul>
			...
			<li><strong>Dvquerytext:</strong> Los valores de este desplegable estan determinados por la consulta:
				<code>
					SELECT fluid_type as id, fluid_type as idval FROM man_type_fluid WHERE ((featurecat_id is null AND 'NODE'=ANY(feature_type)) ) AND active IS TRUE  OR 'CURVE' = ANY(featurecat_id::text[])
				</code>
			</li>
			<li>
				<details class='no-square'>
...
.. raw:: html
	...
	<details>
		...
		<ul>
			...
			<li><strong>Dvquerytext:</strong> Los valores de este desplegable estan determinados por la consulta:
				<code>
					SELECT location_type as id, location_type as idval FROM man_type_location WHERE ((featurecat_id is null AND 'NODE'=ANY(feature_type)) ) AND active IS TRUE  OR 'CURVE' = ANY(featurecat_id::text[])
				</code>
			</li>
			<li>
				<details class='no-square'>
...
.. raw:: html
	...
	<details>
		...
		<ul>
			...
			<li><strong>Dvquerytext:</strong> Los valores de este desplegable estan determinados por la consulta:
				<code>
					SELECT id, id as idval FROM cat_brand WHERE 'CURVE' = ANY(featurecat_id::text[]) OR featurecat_id IS NULL
				</code>
			</li>
			<li>
				<details class='no-square'>
...
.. raw:: html
	...
	<details>
		...
		<ul>
			...
			<li><strong>Dvquerytext:</strong> Los valores de este desplegable estan determinados por la consulta:
				<code>
					SELECT id, id as idval FROM cat_brand_model WHERE 'CURVE' = ANY(featurecat_id::text[]) OR featurecat_id IS NULL
				</code>
			</li>
			<li>
				<details class='no-square'>
