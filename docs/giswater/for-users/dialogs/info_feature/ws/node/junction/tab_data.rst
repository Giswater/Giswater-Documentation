...
.. raw:: html
	...
	<details>
		...
		<ul>
			...
			<li><strong>Dvquerytext:</strong> The values of this dropdown are determined by the query:
				<code>
					SELECT fluid_type as id, fluid_type as idval FROM man_type_fluid WHERE ((featurecat_id is null AND 'NODE'=ANY(feature_type)) ) AND active IS TRUE  OR 'JUNCTION' = ANY(featurecat_id::text[])
				</code>
			</li>
			<li>
				<details class='no-square'>
